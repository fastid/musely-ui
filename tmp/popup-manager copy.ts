import Vue from 'vue'
import { EventEmitter } from 'events'
import { addClass, removeClass } from 'musely-ui/src/utils/dom'

interface ChangeCheckboxEvent extends MouseEvent {
  target: HTMLInputElement
}
interface IPopupManager {
  [any: string]: any
  modalFade: boolean
  modalStack?: any[]
  modalDom?: any
  zIndex?: number
  hasModal: boolean
  getInstance: (id: number) => void
  register: (id: number, instance: number) => void
  deregister: (id: number, instance: number) => void
  nextZIndex: () => void
  doOnModalClick: () => void
  openModal: (id: any, zIndex: any, dom: any, modalClass: any, modalFade: any) => void
  closeModal: (id: number) => void
}
let hasInitZIndex = false
let zIndex = 0

const instances: any = {}

const getTopPopup = function() {
  if (Vue.prototype.$isServer) return
  if (PopupManager.modalStack.length > 0) {
    const topPopup = PopupManager.modalStack[PopupManager.modalStack.length - 1]
    if (!topPopup) return
    const instance = PopupManager.getInstance(topPopup.id)

    return instance
  }
}

if (!Vue.prototype.$isServer) {
  // handle `esc` key when the popup is shown
  window.addEventListener('keydown', function(event) {
    if (event.keyCode === 27) {
      const topPopup = getTopPopup()

      if (topPopup && topPopup.closeOnPressEscape) {
        topPopup.handleClose ? topPopup.handleClose() : topPopup.handleAction ? topPopup.handleAction('cancel') : topPopup.close()
      }
    }
  })
}

export default class PopupManager<IPopupManager> {
  modalFade = true
  modalStack = []
  modalDom: any = null
  hasModal = false
  get zIndex() {
    if (!hasInitZIndex) {
      zIndex = zIndex || (Vue.prototype.$ELEMENT || {}).zIndex || 2000
      hasInitZIndex = true
    }
    return zIndex
  }
  set zIndex(value: any) {
    zIndex = value
  }
  getModal() {
    return this.modalDom
  }
  getInstance(id: any) {
    return instances[id]
  }

  register(id: any, instance: any) {
    if (id && instance) {
      instances[id] = instance
    }
  }

  deregister(id: any) {
    if (id) {
      instances[id] = null
      delete instances[id]
    }
  }

  nextZIndex() {
    return this.zIndex++
  }

  doOnModalClick() {
    const topItem = this.modalStack[this.modalStack.length - 1]
    if (!topItem) return

    const instance = this.getInstance(topItem.id)
    if (instance && instance.closeOnClickModal) {
      instance.close()
    }
  }

  openModal(id: any, zIndex: any, dom: any, modalClass: any, modalFade: any) {
    if (Vue.prototype.$isServer) return
    if (!id || zIndex === undefined) return
    this.modalFade = modalFade

    const modalStack = this.modalStack

    for (let i = 0, j = modalStack.length; i < j; i++) {
      const item = modalStack[i]
      if (item.id === id) {
        return
      }
    }

    const modalDom = () => {
      if (Vue.prototype.$isServer) return
      let modalDom = this.modalDom
      if (modalDom) {
        this.hasModal = true
      } else {
        this.hasModal = false
        this.modalDom = document.createElement('div')
        this.modalDom = modalDom

        modalDom.addEventListener('touchmove', (event: ChangeCheckboxEvent) => {
          event.preventDefault()
          event.stopPropagation()
        })

        modalDom.addEventListener('click', () => {
          this.doOnModalClick && this.doOnModalClick()
        })
      }
    }

    addClass(modalDom, 'v-modal')
    if (this.modalFade && !this.hasModal) {
      addClass(modalDom, 'v-modal-enter')
    }
    if (modalClass) {
      const classArr = modalClass.trim().split(/\s+/)
      classArr.forEach((item: any) => addClass(modalDom, item))
    }
    setTimeout(() => {
      removeClass(modalDom, 'v-modal-enter')
    }, 200)

    if (dom && dom.parentNode && dom.parentNode.nodeType !== 11) {
      dom.parentNode.appendChild(modalDom)
    } else {
      document.body.appendChild(this.modalDom)
    }

    if (zIndex) {
      this.modalDom.style.zIndex = zIndex
    }
    this.modalDom.tabIndex = 0
    this.modalDom.style.display = ''

    this.modalStack.push({ id, zIndex, modalClass })
  }

  closeModal(id: any) {
    const modalStack: any = this.modalStack
    const modalDom = this.getModal()

    if (modalStack.length > 0) {
      const topItem: any = modalStack[modalStack.length - 1]
      if (topItem.id === id) {
        if (topItem.modalClass) {
          const classArr = topItem.modalClass.trim().split(/\s+/)
          classArr.forEach((item: any) => removeClass(modalDom, item))
        }

        modalStack.pop()
        if (modalStack.length > 0) {
          modalDom.style.zIndex = modalStack[modalStack.length - 1].zIndex
        }
      } else {
        for (let i = modalStack.length - 1; i >= 0; i--) {
          if (modalStack[i].id === id) {
            modalStack.splice(i, 1)
            break
          }
        }
      }
    }

    if (modalStack.length === 0) {
      if (this.modalFade) {
        addClass(modalDom, 'v-modal-leave')
      }
      setTimeout(() => {
        if (modalStack.length === 0) {
          if (modalDom.parentNode) modalDom.parentNode.removeChild(modalDom)
          modalDom.style.display = 'none'
          this.modalDom = undefined
        }
        removeClass(modalDom, 'v-modal-leave')
      }, 200)
    }
  }
}
